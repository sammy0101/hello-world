...
export default {
    async fetch(request, env, ctx) {
        ...
        const 訪問IP = request.headers.get('X-Real-IP') || request.headers.get('CF-Connecting-IP') || request.headers.get('X-Forwarded-For') || request.headers.get('True-Client-IP') || request.headers.get('Fly-Client-IP') || request.headers.get('X-Appengine-Remote-Addr') || request.headers.get('X-Forwarded-For') || request.headers.get('X-Real-IP') || request.headers.get('X-Cluster-Client-IP') || request.cf?.clientTcpRtt || '未知IP';
        if (env.GO2SOCKS5) SOCKS5白名單 = await 整理成數組(env.GO2SOCKS5);
        if (!upgradeHeader || upgradeHeader !== 'websocket') {
            ...
            if (!env.KV) return fetch(Pages靜態頁面 + '/noKV').then(r => { const headers = new Headers(r.headers); headers.set('Cache-Control', 'no-store, no-cache, must-revalidate, proxy-revalidate'); headers.set('Pragma', 'no-cache'); headers.set('Expires', '0'); return new Response(r.body, { status: 404, statusText: r.statusText, headers }); });
            const 訪問路徑 = url.pathname.slice(1).toLowerCase();
            const 區分大小寫訪問路徑 = url.pathname.slice(1);
            if (訪問路徑 === 加密秘鑰 && 加密秘鑰 !== '勿動此默認密鑰，有需求請自行通過添加變量KEY進行修改') {//快速訂閱
                ...
                if (request.method === 'POST') {
                    ...
                    if (輸入密碼 === 管理員密碼) {
                        ...
                        響應.headers.set('Set-Cookie', `auth=${await MD5MD5(UA + 加密秘鑰 + 管理員密碼)}; Path=/; Max-Age=86400; HttpOnly`);
                        return 響應;
                    }
                }
                ...
                if (訪問路徑 === 'admin/log.json') {// 讀取日誌內容
                    ...
                    if (url.searchParams.get('url')) {
                        ...
                        try {
                            new URL(待驗證優選URL);
                            const 優選API的IP = await 請求優選API([待驗證優選URL], url.searchParams.get('port') || '443');
                ...
                } else if (訪問路徑 === 'admin/check') {// SOCKS5代理檢查
                    let 檢測代理響應;
                    if (url.searchParams.has('socks5')) {
                        檢測代理響應 = await SOCKS5可用性驗證('socks5', url.searchParams.get('socks5'));
                    } else if (url.searchParams.has('http')) {
                        檢測代理響應 = await SOCKS5可用性驗證('http', url.searchParams.get('http'));
                    } else {
                ...
                config_JSON = await 讀取config_JSON(env, host, userID, env.PATH);

                if (訪問路徑 === 'admin/init') {// 重置配置為默認值
                    try {
                        config_JSON = await 讀取config_JSON(env, host, userID, env.PATH, true);
                ...
                return fetch(Pages靜態頁面 + '/admin');
            } else if (訪問路徑 === 'logout') {//清除cookie並跳轉到登錄頁面
                ...
                const 訂閱TOKEN = await MD5MD5(host + userID);
                if (url.searchParams.get('token') === 訂閱TOKEN) {
                    config_JSON = await 讀取config_JSON(env, host, userID, env.PATH);
                    ctx.waitUntil(請求日誌記錄(env, request, 訪問IP, 'Get_SUB', config_JSON));
                    ...
                    const 協議類型 = (url.searchParams.has('surge') || ua.includes('surge')) ? 'tro' + 'jan' : config_JSON.協議類型;
                    let 訂閱內容 = '';
                    if (訂閱類型 === 'mixed') {
                        const 節點路徑 = config_JSON.啟用0RTT ? config_JSON.PATH + '?ed=2560' : config_JSON.PATH;
                        ...
                        if (!url.searchParams.has('sub') && config_JSON.優選訂閱生成.local) { // 本地生成訂閱
                            const 完整優選列表 = config_JSON.優選訂閱生成.本地IP庫.隨機IP ? (await 生成隨機IP(request, config_JSON.優選訂閱生成.本地IP庫.隨機數量, config_JSON.優選訂閱生成.本地IP庫.指定端口))[0] : await env.KV.get('ADD.txt') ? await 整理成數組(await env.KV.get('ADD.txt')) : (await 生成隨機IP(request, config_JSON.優選訂閱生成.本地IP庫.隨機數量, config_JSON.優選訂閱生成.本地IP庫.指定端口))[0];
                            const 優選API = [], 優選IP = [], 其他節點 = [];
                            for (const 元素 of 完整優選列表) {
                                if (元素.toLowerCase().startsWith('https://')) 優選API.push(元素);
                                else if (元素.toLowerCase().includes('://')) 其他節點.push(元素);
                                else 優選IP.push(元素);
                            }
                            其他節點LINK = 其他節點.join('\n') + '\n';
                            const 優選API的IP = await 請求優選API(優選API);
                            完整優選IP = [...new Set(優選IP.concat(優選API的IP))];
                        } else { // 優選訂閱生成器
                            let 優選訂閱生成器HOST = url.searchParams.get('sub') || config_JSON.優選訂閱生成.SUB;
                            優選訂閱生成器HOST = 優選訂閱生成器HOST && !/^https?:\/\//i.test(優選訂閱生成器HOST) ? `https://${優選訂閱生成器HOST}` : 優選訂閱生成器HOST;
                            const 優選訂閱生成器URL = `${優選訂閱生成器HOST}/sub?host=example.com&uuid=00000000-0000-4000-8000-000000000000`;
                            try {
                                const response = await fetch(優選訂閱生成器URL, { headers: { 'User-Agent': 'v2rayN/edge' + 'tunnel (https://github.com/cmliu/edge' + 'tunnel)' } });
                                if (!response.ok) return new Response('優選訂閱生成器異常：' + response.statusText, { status: response.status });
                                ...
                                for (const 行內容 of 訂閱行列表) {
                                    ...
                                    if (行內容.includes('00000000-0000-4000-8000-000000000000') && 行內容.includes('example.com')) { // 這是優選IP行，提取 域名:端口#備註
                                        const 地址匹配 = 行內容.match(/:\/\/[^@]+@([^?]+)/);
                                        if (地址匹配) {
                                            let 地址端口 = 地址匹配[1], 備註 = ''; // 域名:端口 或 IP:端口
                                ...
                                return new Response('優選訂閱生成器異常：' + error.message, { status: 403 });
                            }
                        }

                        訂閱內容 = 完整優選IP.map(原始地址 => {
                            // 統一正則: 匹配 域名/IPv4/IPv6地址 + 可選端口 + 可選備註
                            // 示例: 
                            //   - 域名: hj.xmm1993.top:2096#備註 或 example.com
                            //   - IPv4: 166.0.188.128:443#Los Angeles 或 166.0.188.128
                            //   - IPv6: [2606:4700::]:443#CMCC 或 [2606:4700::]
                            const regex = /^(\[[\da-fA-F:]+\]|[\d.]+|[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?(?:\.[a-zA-Z0-9](?:[a-zA-Z0-9-]*[a-zA-Z0-9])?)*)(?::(\d+))?(?:#(.+))?$/;
                            const match = 原始地址.match(regex);

                            let 節點地址, 節點端口 = "443", 節點備註;

                            if (match) {
                                節點地址 = match[1];  // IP地址或域名(可能帶方括號)
                                節點端口 = match[2] || "443";  // 端口,默認443
                                節點備註 = match[3] || 節點地址;  // 備註,默認為地址本身
                            } else {
                                // 不規範的格式，跳過處理返回null
                            ...
                            return `${協議類型}://00000000-0000-4000-8000-000000000000@${節點地址}:${節點端口}?security=tls&type=${config_JSON.傳輸協議}&host=example.com&sni=example.com&path=${encodeURIComponent(config_JSON.隨機路徑 ? 隨機路徑() + 節點路徑 : 節點路徑) + TLS分片參數}&encryption=none${config_JSON.跳過證書驗證 ? '&allowInsecure=1' : ''}#${encodeURIComponent(節點備註)}`;
                        }).filter(item => item !== null).join('\n');
                    } else { // 訂閱轉換
                        const 訂閱轉換URL = `${config_JSON.訂閱轉換配置.SUBAPI}/sub?target=${訂閱類型}&url=${encodeURIComponent(url.protocol + '//' + url.host + '/sub?target=mixed&token=' + 訂閱TOKEN + (url.searchParams.has('sub') && url.searchParams.get('sub') != '' ? `&sub=${url.searchParams.get('sub')}` : ''))}&config=${encodeURIComponent(config_JSON.訂閱轉換配置.SUBCONFIG)}&emoji=${config_JSON.訂閱轉換配置.SUBEMOJI}&scv=${config_JSON.跳過證書驗證}`;
                        try {
                            ...
                            if (response.ok) {
                                ...
                                if (url.searchParams.has('surge') || ua.includes('surge')) 訂閱內容 = surge(訂閱內容, url.protocol + '//' + url.host + '/sub?token=' + 訂閱TOKEN + '&surge', config_JSON);
                    ...
                    if (!ua.includes('subconverter')) 訂閱內容 = 批量替換域名(訂閱內容.replace(/00000000-0000-4000-8000-000000000000/g, config_JSON.UUID), config_JSON.HOSTS)

                    if (!ua.includes('mozilla') && 訂閱類型 === 'mixed') 訂閱內容 = btoa(訂閱內容);

                    if (訂閱類型 === 'singbox') {
                        訂閱內容 = JSON.stringify(JSON.parse(訂閱內容), null, 2);
                ...
                return new Response('無效的訂閱TOKEN', { status: 403 });
        ...
        try {
            ...
            if (新請求頭.has('Referer')) { const u = new URL(新請求頭.get('Referer')); 新請求頭.set('Referer', 反代URL.protocol + '//' + 反代URL.host + u.pathname + u.search); }
            if (新請求頭.has('Origin')) 新請求頭.set('Origin', 反代URL.protocol + '//' + 反代URL.host);
            if (!新請求頭.has('User-Agent') && UA && UA !== 'null') 新請求頭.set('User-Agent', UA);
            return fetch(new Request(反代URL.protocol + 反代URL.host + url.pathname + url.search, { method: request.method, headers: 新請求頭, body: request.body, cf: request.cf }));
        } catch (error) { }
...
async function 處理WS請求(request, yourUUID) {
    ...
    readable.pipeTo(new WritableStream({
        async write(chunk) {
            ...
            if (判斷是否是木馬) {
                const { port, hostname, rawClientData } = 解析木馬請求(chunk, yourUUID);
                if (isSpeedTestSite(hostname)) throw new Error('Speedtest site is blocked');
                await forwardataTCP(hostname, port, rawClientData, serverSock, null, remoteConnWrapper);
            } else {
                const { port, hostname, rawIndex, version, isUDP } = 解析魏烈思請求(chunk, yourUUID);
                if (isSpeedTestSite(hostname)) throw new Error('Speedtest site is blocked');
                if (isUDP) {
...
async function forwardataTCP(host, portNum, rawData, ws, respHeader, remoteConnWrapper) {
    console.log(JSON.stringify({ configJSON: { 目標地址: host, 目標端口: portNum, 反代IP: 反代IP, 代理類型: 啟用SOCKS5反代, 全局代理: 啟用SOCKS5全局反代, 代理賬號: 我的SOCKS5賬號 } }));
    async function connectDirect(address, port, data, 所有反代數組 = null) {
        let remoteSock;
        if (所有反代數組 && 所有反代數組.length > 0) {
            const 打亂後數組 = [...所有反代數組].sort(() => Math.random() - 0.5);
            const 最大嘗試次數 = Math.min(8, 打亂後數組.length);
            for (let i = 0; i < 最大嘗試次數; i++) {
                const [反代地址, 反代端口] = 打亂後數組[i];
                try {
    ...
    async function connecttoPry() {
        ...
        } else if (啟用SOCKS5反代 === 'http' || 啟用SOCKS5反代 === 'https') {
            newSocket = await httpConnect(host, portNum, rawData);
        } else {
            const 所有反代數組 = await 解析地址端口(反代IP);
            newSocket = await connectDirect(atob('UFJPWFlJUC50cDEuMDkwMjI3Lnh5eg=='), 1, rawData, 所有反代數組);
        }
    ...
    if (啟用SOCKS5反代 && (啟用SOCKS5全局反代 || 驗證SOCKS5白名單(host))) {
        try {
            await connecttoPry();
        } catch (err) {
            throw err;
...
function surge(content, url, config_JSON) {
    ...
    for (let x of 每行內容) {
        if (x.includes('= tro' + 'jan,')) {
            ...
            const 正確內容 = `sni=${host}, skip-cert-verify=${config_JSON.跳過證書驗證}, ws=true, ws-path=${config_JSON.PATH}, ws-headers=Host:"${host}"`;
...
async function 請求日誌記錄(env, request, 訪問IP, 請求類型 = "Get_SUB", config_JSON) {
    const KV容量限制 = 4;//MB
    try {
        const 當前時間 = new Date();
        ...
        let 日誌數組 = [];
        const 現有日誌 = await env.KV.get('log.json');
        if (現有日誌) {
            try {
...
async function sendMessage(BotToken, ChatID, 日誌內容, config_JSON) {
    ...
    try {
        ...
        const msg = `<b>#${config_JSON.優選訂閱生成.SUBNAME} 日誌通知</b>\n\n` +
            ...
            `📅 <b>時間：</b>${請求時間}\n` +
            `${config_JSON.CF.Usage.success ? `📊 <b>請求用量：</b>${config_JSON.CF.Usage.total}/100000 <b>${((config_JSON.CF.Usage.total / 100000) * 100).toFixed(2)}%</b>\n` : ''}`;
...
function 批量替換域名(內容, hosts, 每組數量 = 2) {
    let count = 0, currentRandomHost = null;
    return 內容.replace(/example\.com/g, () => {
        if (count % 每組數量 === 0) currentRandomHost = 隨機替換通配符(hosts[Math.floor(Math.random() * hosts.length)]);
        count++;
        return currentRandomHost;
    });
}

async function 讀取config_JSON(env, hostname, userID, path, 重置配置 = false) {
    //const host = 隨機替換通配符(hostname);
    const host = hostname;
    const 初始化開始時間 = performance.now();
    const 默認配置JSON = {
    ...
    config_JSON.PATH = path ? (path.startsWith('/') ? path : '/' + path) : (config_JSON.反代.SOCKS5.啟用 ? ('/' + config_JSON.反代.SOCKS5.啟用 + (config_JSON.反代.SOCKS5.全局 ? '://' : '=') + config_JSON.反代.SOCKS5.賬號) : (config_JSON.反代.PROXYIP === 'auto' ? '/' : `/proxyip=${config_JSON.反代.PROXYIP}`));
    const TLS分片參數 = config_JSON.TLS分片 == 'Shadowrocket' ? `&fragment=${encodeURIComponent('1,40-60,30-50,tlshello')}` : config_JSON.TLS分片 == 'Happ' ? `&fragment=${encodeURIComponent('3,1,tlshello')}` : '';
    config_JSON.LINK = `${config_JSON.協議類型}://${userID}@${host}:443?security=tls&type=${config_JSON.傳輸協議}&host=${host}&sni=${host}&path=${encodeURIComponent(config_JSON.啟用0RTT ? config_JSON.PATH + '?ed=2560' : config_JSON.PATH) + TLS分片參數}&encryption=none${config_JSON.跳過證書驗證 ? '&allowInsecure=1' : ''}#${encodeURIComponent(config_JSON.優選訂閱生成.SUBNAME)}`;
...
async function 生成隨機IP(request, count = 16, 指定端口 = -1) {
    ...
    const cfname = { '9808': 'CF移動優選', '4837': 'CF聯通優選', '4134': 'CF電信優選' }[asn] || 'CF官方優選';
        ...
        try {
            ...
            try {
                ...
                if (charset.includes('gb') || charset.includes('gbk') || charset.includes('gb2312')) {
                    decoders = ['gb2312', 'utf-8']; // 如果明確指定 GB 系編碼，優先嚐試 GB2312
                }
            ...
            if (!isCSV) {
                lines.forEach(line => {
                    ...
                    const port = new URL(url).searchParams.get('port') || 默認端口;
                ...
                if (headers.includes('IP地址') && headers.includes('端口') && headers.includes('數據中心')) {
                    ...
                    dataLines.forEach(line => {
                        const cols = line.split(',').map(c => c.trim());
                        const wrappedIP = IPV6_PATTERN.test(cols[ipIdx]) ? `[${cols[ipIdx]}]` : cols[ipIdx];
                        results.add(`${wrappedIP}:${port}#CF優選 ${cols[delayIdx]}ms ${cols[speedIdx]}MB/s`);
                    });
                }
            }
        } catch (e) { }
...
async function 反代參數獲取(request) {
    const url = new URL(request.url);
    const { pathname, searchParams } = url;
    const pathLower = pathname.toLowerCase();

    // 初始化
    我的SOCKS5賬號 = searchParams.get('socks5') || searchParams.get('http') || null;
    啟用SOCKS5全局反代 = searchParams.has('globalproxy') || false;

    // 統一處理反代IP參數 (優先級最高,使用正則一次匹配)
    const proxyMatch = pathLower.match(/\/(proxyip[.=]|pyip=|ip=)(.+)/);
    if (searchParams.has('proxyip')) {
        const 路參IP = searchParams.get('proxyip');
        反代IP = 路參IP.includes(',') ? 路參IP.split(',')[Math.floor(Math.random() * 路參IP.split(',').length)] : 路參IP;
        return;
    } else if (proxyMatch) {
        const 路參IP = proxyMatch[1] === 'proxyip.' ? `proxyip.${proxyMatch[2]}` : proxyMatch[2];
        反代IP = 路參IP.includes(',') ? 路參IP.split(',')[Math.floor(Math.random() * 路參IP.split(',').length)] : 路參IP;
        return;
    }
    ...
    if ((socksMatch = pathname.match(/\/(socks5?|http):\/?\/?(.+)/i))) {
        // 格式: /socks5://... 或 /http://...
        啟用SOCKS5反代 = socksMatch[1].toLowerCase() === 'http' ? 'http' : 'socks5';
        我的SOCKS5賬號 = socksMatch[2].split('#')[0];
        啟用SOCKS5全局反代 = true;

        // 處理Base64編碼的用戶名密碼
        if (我的SOCKS5賬號.includes('@')) {
            const atIndex = 我的SOCKS5賬號.lastIndexOf('@');
            let userPassword = 我的SOCKS5賬號.substring(0, atIndex).replaceAll('%3D', '=');
            if (/^(?:[A-Z0-9+/]{4})*(?:[A-Z0-9+/]{2}==|[A-Z0-9+/]{3}=)?$/i.test(userPassword) && !userPassword.includes(':')) {
                userPassword = atob(userPassword);
            }
            我的SOCKS5賬號 = `${userPassword}@${我的SOCKS5賬號.substring(atIndex + 1)}`;
        }
        ...
        我的SOCKS5賬號 = socksMatch[2];
        啟用SOCKS5反代 = type.includes('http') ? 'http' : 'socks5';
        啟用SOCKS5全局反代 = type.startsWith('g') || 啟用SOCKS5全局反代; // gs5 或 ghttp 開頭啟用全局
    }

    // 解析SOCKS5地址
    if (我的SOCKS5賬號) {
        try {
            parsedSocks5Address = await 獲取SOCKS5賬號(我的SOCKS5賬號);
            啟用SOCKS5反代 = searchParams.get('http') ? 'http' : 啟用SOCKS5反代;
        } catch (err) {
            console.error('解析SOCKS5地址失敗:', err.message);
            啟用SOCKS5反代 = null;
        }
    } else 啟用SOCKS5反代 = null;
}

async function 獲取SOCKS5賬號(address) {
    if (address.includes('@')) {
    ...
    if (authPart) {
        ...
        if (!password) throw new Error('無效的 SOCKS 地址格式：認證部分必須是 "username:password" 的形式');
    }

    // 解析主機端口
    let hostname, port;
    if (hostPart.includes("]:")) { // IPv6帶端口
        [hostname, port] = [hostPart.split("]:")[0] + "]", Number(hostPart.split("]:")[1].replace(/[^\d]/g, ''))];
    } else if (hostPart.startsWith("[")) { // IPv6無端口
    ...
    if (hostname.includes(":") && !/^\[.*\]$/.test(hostname)) throw new Error('無效的 SOCKS 地址格式：IPv6 地址必須用方括號括起來，如 [2001:db8::1]');
...
async function getCloudflareUsage(Email, GlobalAPIKey, AccountID, APIToken) {
    ...
    try {
        ...
        console.log(`統計結果 - Pages: ${pages}, Workers: ${workers}, 總計: ${total}`);
...
async function 解析地址端口(proxyIP) {
    ...
    async function DoH查詢(域名, 記錄類型) {
        try {
            const response = await fetch(`https://1.1.1.1/dns-query?name=${域名}&type=${記錄類型}`, {
            ...
            console.error(`DoH查詢失敗 (${記錄類型}):`, error);
            return [];
        }
    }
    ...
    if (proxyIP.includes('.william')) {
        try {
            const txtRecords = await DoH查詢(proxyIP, 'TXT');
            const txtData = txtRecords.filter(r => r.type === 16).map(r => r.data);
            if (txtData.length > 0) {
                let data = txtData[0];
                if (data.startsWith('"') && data.endsWith('"')) data = data.slice(1, -1);
                const prefixes = data.replace(/\\010/g, ',').replace(/\n/g, ',').split(',').map(s => s.trim()).filter(Boolean);
                所有反代數組 = prefixes.map(prefix => 解析地址端口字符串(prefix));
            }
        } catch (error) {
            console.error('解析William域名失敗:', error);
        }
    } else {
    ...
    return 所有反代數組;
    // low
    const [選中地址, 選中端口] = 所有反代數組[Math.floor(Math.random() * 所有反代數組.length)];
    return [選中地址, 選中端口];
}

async function SOCKS5可用性驗證(代理協議 = 'socks5', 代理參數) {
    const startTime = Date.now();
    try { parsedSocks5Address = await 獲取SOCKS5賬號(代理參數); } catch (err) { return { success: false, error: err.message, proxy: 代理協議 + "://" + 代理參數, responseTime: Date.now() - startTime }; }
    const { username, password, hostname, port } = parsedSocks5Address;
    const 完整代理參數 = username && password ? `${username}:${password}@${hostname}:${port}` : `${hostname}:${port}`;
    try {
        const initialData = new Uint8Array(0);
        const tcpSocket = 代理協議 == 'socks5' ? await socks5Connect('check.socks5.090227.xyz', 80, initialData) : await httpConnect('check.socks5.090227.xyz', 80, initialData);
        if (!tcpSocket) return { success: false, error: '無法連接到代理服務器', proxy: 代理協議 + "://" + 完整代理參數, responseTime: Date.now() - startTime };
        try {
            ...
            return { success: true, proxy: 代理協議 + "://" + 完整代理參數, ip: response.match(/ip=(.*)/)[1], loc: response.match(/loc=(.*)/)[1], responseTime: Date.now() - startTime };
        } catch (error) {
            try { await tcpSocket.close(); } catch (e) { console.log('關閉連接時出錯:', e); }
            return { success: false, error: error.message, proxy: 代理協議 + "://" + 完整代理參數, responseTime: Date.now() - startTime };
        }
    } catch (error) { return { success: false, error: error.message, proxy: 代理協議 + "://" + 完整代理參數, responseTime: Date.now() - startTime }; }
}
//////////////////////////////////////////////////////HTML偽裝頁面///////////////////////////////////////////////
